"""Real-time stereo needle keypoint + 6-DoF pose — ACCELERATED (v3).

Same segmentation + v2 model-based pose as realtime_stereo_keypoints_v2.py, plus
inference-time speedups (see infer_accel.py / docs):
  --compile / --channels-last : torch.compile + channels_last on the seg forward
  --seg-engine PATH           : use a pre-exported TorchScript/TensorRT engine
  async I/O                   : background frame prefetch + video encode (always on,
                                disable with --no-async)
Accuracy is unchanged (same fp16 math); only the forward/throughput differ.
...
def load_nk(path):
    ...
    import needle_keypoints_v2 as nk     # v2: model-based (fixed-radius) pose registration
    return nk
...
def main():
    ...
    p.add_argument('--seg-engine', default=None,
                   help='pre-exported TorchScript/TensorRT engine (from export_seg_engine.py); '
                        'biggest forward-pass speedup. Overrides --compile/--channels-last.')
    p.add_argument('--no-async', action='store_true',
                   help='disable background frame-prefetch + async video encode')
    args = p.parse_args()
    ...
    if args.needle_model and os.path.isfile(args.needle_model):
        ...
        args.model_radius = float(_json.loads(
            open(args.needle_model, encoding='utf-8').read())['radius_mm'])
    if args.model_radius:
        print(f'[realtime] model-based pose: fixed needle radius = {args.model_radius:.2f} mm')
