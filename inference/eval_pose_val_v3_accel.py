"""Evaluate seg-model needle keypoints + 6-DoF pose on the VAL stereo videos — ACCELERATED (v3).

Same metrics + v2 model-based pose as eval_pose_val_v2.py, plus inference-time
speedups (see infer_accel.py): --compile / --channels-last / --seg-engine on the
seg forward, background frame prefetch (--no-async to disable), and --no-video to
skip canvas drawing + mp4 encoding entirely when you only want the numbers.
...
from realtime_stereo_keypoints_v3_accel import (   # v3: accel helpers + load_nk(needle_keypoints_v2)
...
def run_key(dataset, key, bundle, nk, calib, rvecR, tvecR, kp_names, args, device, use_amp):
    ...
    if not args.no_async:                       # v3: overlap disk decode with the GPU forward
    ...
    while True:
        ...
        # ---- visualization / video (skip entirely with --no-video: numbers-only, faster) ----
        if not args.no_video:
            ...
            if writer is None:
                ...
                writer = (cv2.VideoWriter(out_path, fourcc, float(args.fps_out), (w, h))
                          if args.no_async else
                          infer_accel.AsyncVideoWriter(out_path, fourcc, float(args.fps_out), (w, h)))
            writer.write(canvas)
...
def main():
    ...
    p.add_argument('--model-radius', type=float, default=None,
                   help='known needle arc radius (mm) -> fixed-radius 3D registration (model-based pose)')
    ...
    # ---- v3 inference-time acceleration ----
    p.add_argument('--compile', action='store_true',
                   help='torch.compile the seg forward (1.2-1.6x; first frames slow while tracing)')
    p.add_argument('--channels-last', action='store_true',
                   help='channels_last memory format on the seg forward (helps the DPT conv head)')
    p.add_argument('--seg-engine', default=None,
                   help='pre-exported TorchScript/TensorRT engine (export_seg_engine.py); '
                        'overrides --compile/--channels-last')
    p.add_argument('--no-async', action='store_true', help='disable frame prefetch + async encode')
    p.add_argument('--no-video', action='store_true',
                   help='skip canvas drawing + mp4 encoding (numbers-only; fastest)')
    args = p.parse_args()
    ...
    if args.model_radius:
        print(f'[eval] model-based pose: fixed needle radius = {args.model_radius:.2f} mm')
    ...
    # ---- v3: accelerated segmentation backend ----
    if args.seg_engine:
        infer_accel.attach_engine_to_bundle(bundle, args.seg_engine, device)
    elif args.compile or args.channels_last:
        infer_accel.accelerate_bundle(bundle, compile=args.compile,
